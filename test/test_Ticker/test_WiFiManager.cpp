...
TEST_F(WiFiManagerTest, data)
{
    ...
    EXPECT_GT(wm.getCurrentPrice("BTC", "USD"), 0);
    uint32_t currentTime = wm.getCurrentTime();

    // see compile_time.h
    // expect time is within 24h of test compile time    
    EXPECT_GT(currentTime, UNIX_TIMESTAMP-SEC_PER_DAY);
    EXPECT_LT(currentTime, UNIX_TIMESTAMP+SEC_PER_DAY);
}
